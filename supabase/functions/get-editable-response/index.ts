...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const id = cleanUuid(body.id); const token = cleanCredential(body.token, 43);
    const tokenHmac = await hmacHex(requiredSecret('EDIT_TOKEN_HMAC_SECRET'), token);
    const result = await adminClient().rpc('get_editable_response_internal', { response_id_value: id, token_hmac: tokenHmac });
    if (result.error || !result.data) throw new Error('not_found_or_invalid_token');
    return json(request, { response: result.data });
  } catch (error) {
